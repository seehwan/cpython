...
combo_gadget = b"\x50\x00\x00\x58" + b"\x00\x02\x1f\xd6"
magic_values = [
    0x0000FFFF,
    0xFFFF0000,
    0xF0F0F0F0,
...
def make_combo_func(magic):
    def combo(x: int):
        ...
        for i in range(3000):
            acc ^= (magic & i)
            acc += 0x58000050  # encoding for ldr x16, [pc, #8]
            acc += 0xd61f0200  # encoding for br x16
            acc += (acc << 1)
            acc &= 0xFFFFFFFFFFFFFFFF  # ensure 64-bit wraparound
        return acc
    return combo
