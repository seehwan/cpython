...
def scan_region_all_gadgets(start, end, magic):
    print(f"[*] Scanning: 0x{start:x}-0x{end:x} for magic 0x{magic:08x}")
    ...
    for i in range(len(mem) - 4):
        ...
        if chunk in raw_gadgets:
            addr = start + i
            print(f"[GADGET][0x{magic:08x}] {raw_gadgets[chunk]} @ 0x{addr:x}")
            found_any = True
    if not found_any:
        print(f"[0x{magic:08x}] No gadgets found in region 0x{start:x}-0x{end:x}")
...
def spray_and_scan(magic):
    ...
    for start, end in get_jit_regions():
        scan_region_all_gadgets(start, end, magic)
